      ...
      <div class="song-list">
        ${e.map(t=>L`
          <div class="song-item-compact">
            <span class="song-name-compact">${t.name}</span>
            ${t.duration?L`<span class="song-duration-compact">${this._formatDuration(t.duration/1e3)}</span>`:""}
          </div>
        `)}
      </div>
    `:L`
      ...
      `}async _playPlaylist(t,e){if(t.stopPropagation(),this.config.confirm_play){if(!confirm(`Play playlist "${e}"?`))return}try{await this._hass.callService("media_player","play_media",{entity_id:this.config.entity,media_content_type:"playlist",media_content_id:e})}catch(t){console.error("Failed to play playlist:",t)}}getCardSize(){return this.config.compact_mode?4:6}static get styles(){return o`
      :host {
        display: block;
      }
