...
async def async_setup_entry(
    ...
    if not media_player_entity or not hasattr(media_player_entity, "_controller_status"):
        _LOGGER.warning("xSchedule media player not found or has no controller status")
        return
    ...
    def handle_controller_update(event):
        ...
        current_sensors = {sensor.unique_id: sensor for sensor in sensors}

        # Update existing sensors or create new ones
        new_sensors = []
        for controller_data in controllers:
            if "controller" in controller_data and "ip" in controller_data:
                # Generate unique_id to check if sensor exists
                ...
                if unique_id in current_sensors:
                    ...
                    current_sensors[unique_id].update_from_data(controller_data)
                else:
                    # Create new sensor
